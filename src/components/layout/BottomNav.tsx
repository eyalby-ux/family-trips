...
const navItems: { id: Screen; label: string; icon: string }[] = [
  ...
  { id: "useful", label: "מסביב", icon: "🧭" },
  { id: "games", label: "משחקים", icon: "🎮" },
];

function BottomNav({ activeScreen, onChange }: BottomNavProps) {
  return <nav className="bottomNav">{navItems.map((item) => <button key={item.id} className={activeScreen === item.id ? "active" : ""} onClick={() => onChange(item.id)}><span>{item.icon}</span><small>{item.label}</small></button>)}</nav>;
}
