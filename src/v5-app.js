...
function detailView(){const item=state.items.find(candidate=>candidate.id===state.itemId);if(!item)return '<section class="card">הפריט לא נמצא.</section>';const sources=state.sources.filter(source=>item.sourceIds?.includes(source.id)),place=item.details?.canonicalPlace,contactActions=[place?`<a class="btn secondary" href="${esc(place.googleMapsUri||mapsUrl(`${place.latitude},${place.longitude}`))}" target="_blank" rel="noopener">🧭 ניווט</a>`:item.location?`<a class="btn secondary" href="${mapsUrl(item.location)}" target="_blank" rel="noopener">🧭 חיפוש במפה</a>`:'',item.website?`<a class="btn ghost" href="${esc(item.website)}" target="_blank" rel="noopener">🌐 קישור</a>`:'',item.phone?`<a class="btn ghost" href="tel:${esc(item.phone)}">☎️ התקשרות</a>`:''].filter(Boolean).join('');return `<section class="card ${conflict(item)?'conflict':''}"><button class="btn ghost" data-route="center" data-cat="${item.type}">חזרה</button><h2>${ITEM_TYPES[item.type]?.icon||'📌'} ${esc(item.title)}</h2><p><span class="badge">${ITEM_TYPES[item.type]?.label}</span></p>${item.provider?`<p><b>ספק:</b> ${esc(item.provider)}</p>`:''}${item.confirmationNumber?`<p><b>מספר הזמנה:</b> ${esc(item.confirmationNumber)}</p>`:''}${item.location?`<p><b>מיקום:</b> ${esc(item.location)} ${place?'<span class="badge blue">אומת</span>':'<span class="badge warn">שם לחיפוש</span>'}</p>`:''}${contactActions?`<div class="actions">${contactActions}</div>`:''}${item.startAt?`<p><b>מועד:</b> ${fmt(item.startAt)}</p>`:''}${item.endAt?`<p><b>סיום:</b> ${fmt(item.endAt)}</p>`:''}${item.notes?`<p class="preserve-lines">${esc(item.notes)}</p>`:''}${item.warnings?.length?`<div class="warning-list">${[...new Set(item.warnings)].map(warning=>`<div>⚠️ ${esc(warningText(warning))}</div>`).join('')}</div>`:''}${item.type==='flight'?flightSummaryBlock(item.details):''}<h3>מסמכים וכרטיסים</h3>${sources.length?`<div class="list">${sources.map(sourceRow).join('')}</div>`:'<p class="muted">אין מסמכים מצורפים.</p>'}<div class="actions">${['hotel','flight'].includes(item.type)?`<button class="btn" data-action="attach-source" data-id="${item.id}">הוספת מסמך</button>`:''}<button class="btn secondary" data-action="edit-item" data-id="${item.id}">עריכה</button><button class="btn danger" data-action="delete-item" data-id="${item.id}">מחיקה</button></div></section>`}
...
// departure/arrival airport details all switch together, exactly as one linked group, and the
// generic "כיוון הטיסה (דורש בחירה)" needs-review warning clears now that a choice has been made.
// A later manual edit in the review form (or picking the other candidate) can still override this.
// V6-F35 (fix pass 4): the old filter only matched the app's own Hebrew label and missed a
// model-authored English warning ("Direction ambiguity prevents assigning...") that also
// mentioned the ambiguity, so it kept showing after a pick -- clearResolvedDirectionWarning
// matches both.
function pickFlightDirection(suggestion,index){
  ...
  suggestion.proposed={...suggestion.proposed,location:candidate.label,startAt:candidate.startAt,endAt:candidate.endAt,details:{...suggestion.proposed.details,departureAirport:candidate.departureAirport,arrivalAirport:candidate.arrivalAirport,directionPickIndex:index},warnings:clearResolvedDirectionWarning(suggestion.proposed.warnings)};
  suggestion.warnings=suggestion.proposed.warnings;
  suggestion.updatedAt=new Date().toISOString();
  save();
}
function updateSuggestion(form,suggestion){const data=new FormData(form),type=String(data.get('type')),range=normalizeDateRange(data.get('startAt'),data.get('endAt'),{allowEndBeforeStart:type==='flight'});suggestion.proposed={...suggestion.proposed,type,title:String(data.get('title')||'').trim(),provider:String(data.get('provider')||'').trim(),confirmationNumber:String(data.get('confirmationNumber')||'').trim(),location:String(data.get('location')||'').trim(),website:normalizeUrlInput(data.get('website')),phone:String(data.get('phone')||'').trim(),startAt:range.start,endAt:range.end,participants:String(data.get('participants')||'').split(',').map(x=>x.trim()).filter(Boolean),notes:String(data.get('notes')||'').trim(),schedule:ITEM_TYPES[type]?.schedule||'none'};suggestion.updatedAt=new Date().toISOString();suggestion.status='pending'}
...
addEventListener('submit',event=>{event.preventDefault();const form=event.target,data=new FormData(form);
  ...
  else if(form.id==='manual-form'||form.id==='edit-item-form'){const type=String(data.get('type')),values=buildItemFormValues(data);ensureTrip();if(!confirmOutOfRangeSave(values)){if(form.id==='edit-item-form'){state.modal={type:'edit-item',itemId:form.dataset.id};render()}return}if(form.id==='manual-form')state.items.push({id:uid('item'),...values,sourceIds:[]});else Object.assign(state.items.find(x=>x.id===form.dataset.id),values);save();state.modal=null;route(form.id==='manual-form'?'center':'detail',form.id==='manual-form'?{category:type}:{itemId:form.dataset.id})}
