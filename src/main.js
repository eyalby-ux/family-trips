  ...
  authRoot.innerHTML = `
    <main class="auth-page">
      <section class="auth-card" aria-labelledby="auth-title">
        ...
        <p class="auth-kicker">V4 — Smart Ingestion</p>
        ...
        <p class="auth-note">נתוני הטיול והמקורות נשמרים מקומית במכשיר. Firestore ו־Storage נשארים נעולים.</p>
      </section>
    </main>`;
...
async function renderSignedIn(user) {
  ...
  document.querySelector('#sign-out')?.addEventListener('click', async () => { await signOut(auth); location.reload(); });
  if (!appLoaded) { appLoaded = true; await import('./v4-app.js'); }
}
...
async function start() {
  ...
  onAuthStateChanged(auth, (user) => { if (user) renderSignedIn(user); else renderLogin(); });
