...
function ActionLink({ href, children }: { href: string; children: string }) {
  return (
    <a className="hubAction" href={href} target="_blank" rel="noreferrer">
      {children}
    </a>
  );
}
  ...
  return (
    <div className="liveField">
      <div>
        ...
        <span>{displayValue ? (field.secret ? "••••••••" : displayValue) : "אפשר לעדכן כשתדעו"}</span>
      </div>
      ...
      {isEditing && (
        <div className="liveEdit">
          <input value={value} onChange={(event) => setValue(event.target.value)} placeholder={field.placeholder} />
          <button className="primaryButton" onClick={handleSave}>שמור לכולם</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="hubStack">
      ...
      <article className="hubDetailCard">
        <div className="hubDetailHeader">
          <span className="hubIcon">🏨</span>
          <div>
            <h3>{hotel.name}</h3>
            <span>{hotel.dates} · {hotel.area}</span>
          </div>
        </div>
        ...
        <div className="hubInfoGrid">
          <div><strong>כתובת</strong><span>{hotel.address}</span></div>
          <div><strong>צ׳ק-אין</strong><span>{hotel.checkIn ?? "לעדכן"}</span></div>
          <div><strong>צ׳ק-אאוט</strong><span>{hotel.checkOut ?? "לעדכן"}</span></div>
        </div>
        ...
        <div className="liveNotesBox">
          ...
          {savedMessage && <span className="savedMessage">{savedMessage}</span>}
          {error && <span className="errorInline">{error}</span>}
        </div>

        {hotel.notes && hotel.notes.length > 0 && (
          <details className="hubDetails">
            <summary>הערות מתוכננות</summary>
            <ul>{hotel.notes.map((note) => <li key={note}>{note}</li>)}</ul>
          </details>
        )}
      </article>
    </div>
  ...
  return (
    <div className="hubStack">
      ...
      <div className="compactList">
        {travelHubHotels.map((hotel) => (
          <button key={hotel.id} className="compactHotelCard" onClick={() => setSelectedHotelId(hotel.id)}>
            <span className="compactDate">{hotel.dates}</span>
            <div><strong>{hotel.name}</strong><small>{hotel.area}</small></div>
  ...
  return (
    <div className="hubStack">
      <article className="hubDetailCard">
        ...
        <div className="hubInfoGrid">
          <div><strong>הזמנה</strong><span>{travelHubCar.bookingNumber}</span></div><div><strong>אישור</strong><span>{travelHubCar.confirmationNumber}</span></div><div><strong>נהג ראשי</strong><span>{travelHubCar.mainDriver}</span></div><div><strong>איסוף</strong><span>{travelHubCar.pickupDate} · {travelHubCar.pickupTime}</span></div><div><strong>החזרה</strong><span>{travelHubCar.dropoffDate} · {travelHubCar.dropoffTime}</span></div><div><strong>פיקדון</strong><span>{travelHubCar.deposit}</span></div>
        </div>
        ...
        <details className="hubDetails"><summary>חשוב לדעת</summary><ul>{travelHubCar.importantReminders.map((item) => <li key={item}>{item}</li>)}</ul></details>
      </article>
    </div>
  );
}
...
function TravelHub() {
  ...
  return (
    <section className="section travelHub">
      ...
      {section === "home" && (<><div className="travelersPill">{tripTravelers.map((person) => <span key={person}>{person}</span>)}</div><div className="hubModuleGrid">{moduleCards.map((card) => <button key={card.id} className="hubModuleCard" onClick={() => setSection(card.id)}><span>{card.icon}</span><strong>{card.title}</strong><small>{card.subtitle}</small></button>)}</div></>)}
      {section === "hotels" && <HotelsSection />}
      {section === "car" && <CarSection />}
      {section === "flights" && (<div className="hubStack"><h3>✈️ טיסות</h3>{travelHubFlights.map((flight) => <FlightCard key={flight.id} flight={flight} />)}</div>)}
