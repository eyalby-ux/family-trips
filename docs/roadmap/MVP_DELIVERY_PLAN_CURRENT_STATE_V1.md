...
Two findings discovered during Eyal's own `0.6.6` QA acceptance testing (build `0.6.6-3925bb20`), investigated and registered per the standard's mandatory finding register. Both are now **implemented on `version/V6`, each with a passing named regression test** — but per the standard's responsibility split, neither is recorded `Fixed` yet: these were discovered by the Product Owner's own QA use, not by code review or adapter testing, so closing them requires the Product Owner to actually retest against a QA deploy carrying the fix, the same discipline already applied to `V6-F49`–`V6-F51`. Target version remains `TBD after acceptance completion` for both; no correction package has been authored.
...
Neither finding has a regression test yet — per the standard's V23 rule, neither may be recorded `Fixed` until one exists. `V6-F52`'s is straightforward (source-inspection style, matching this codebase's established pattern for DOM/Firebase-dependent UI code that can't run in the plain-Node test harness). `V6-F53`'s is not yet proposable with confidence: live-model non-determinism isn't reproducible or assertable in a suite with no live API access, so the achievable test depends on which mitigation approach is chosen first — see the Development Agent's investigation report for candidate approaches awaiting Product Owner decision. No fix has been implemented for either finding; investigation and registration only, per explicit instruction.
