...
| `FL-002` | EL AL app screenshot | App screenshot, single-passenger view | Flight LY087, HKT→TLV, departure 14:30 8 Jan 2027, arrival 22:20 7 Jan 2027 (arrival local date earlier than departure local date — a real timezone-crossing case; must not be "corrected" or rejected as invalid). Seat 41C, passenger Eyal Ben Yitzhak. |
| `FL-003` | EL AL app itinerary summary | App screenshot, combined view | Both legs in one view with fare class code and aircraft type ("Dreamliner"), no passenger name shown — tests associating a summary source with the per-leg detail in `FL-001`/`FL-002` without duplicating items. **Direction labels for both legs are unconfirmed pending Product Owner QA verification** — `FL-001`'s direction was miscatalogued from this same screenshot family and corrected in `V2`; treat this case's expected directions as provisional until directly confirmed. |
