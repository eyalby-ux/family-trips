...
|---|---|---|
| V4 | 2026-09-05 | **`V6-F72` (confirmation).** `FL-002`'s direction, corrected in `V3` as an inferred-pending-confirmation correction, is now independently confirmed by the Product Owner: **LY087 is TLV→HKT** (departure TLV 22:20 on 7 Jan 2027, arrival HKT 14:30 on 8 Jan 2027), exactly matching the inferred correction. No longer flagged as pending — this is confirmed ground truth, on the same footing as `FL-001`/`FL-003`. |
...
| `FL-002` | EL AL app screenshot | App screenshot, single-passenger view | Flight LY087, **TLV→HKT** (corrected in `V3`, **confirmed by the Product Owner in `V4`**), departure TLV 22:20 on 7 Jan 2027, arrival HKT 14:30 on 8 Jan 2027 (arrival local date later than departure local date once corrected — a real timezone-crossing case; must not be "corrected" or rejected as invalid). Seat 41C, passenger Eyal Ben Yitzhak. |
